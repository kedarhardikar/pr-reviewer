...
def build_fix_task(agent: Agent) -> Task:
    return Task(
        description=(
            ...
            "Head ref: {head_ref}\n\n"
            "Findings:\n{all_findings_json}\n\n"
            "Changed functions (at HEAD — line numbers here are authoritative):\n"
            "{changed_functions_source}\n\n"
            "IMPORTANT:\n"
            "  - `extract_function_context` is a TOOL you call to READ source code. "
            "It is NOT a function you should write or suggest creating.\n"
            "  - Line numbers in findings refer to the HEAD file, not the diff.\n"
            "  - Call `extract_function_context` FIRST before writing any fix.\n"
            "  - Do NOT invent issues or functions not present in the findings.\n\n"
            "For each finding with a clear fix:\n"
            "  - Call `extract_function_context` to read the function body.\n"
            "  - Produce a minimal corrected snippet.\n"
            "  - Set 'line' to match the finding's line number exactly.\n"
            "  - Skip documentation-only findings (missing docstrings, missing param docs) "
            "— those don't need code changes.\n"
            "  - Skip findings where no clear code fix exists."
        ),
        expected_output=(
